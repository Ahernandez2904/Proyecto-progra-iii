...
public class RutaRepositorio {
    ...
    public Boolean crearRuta(Ruta ruta) {  //clase persona, en clase persona vienen los mismas variables
        try {
            String consulta = "INSERT INTO Ruta (IDRuta, HORARIO)"
    ...
    //READ
    public Ruta leerRuta(String identificacion) {  //(String identificacion)vamos a obtener de la fila que coincida con el valor que le estemos poniendo en en string y en sentencia.setString
        Ruta ruta = null;
        try {
            String consulta = "SELECT IDRUTA, HORARIO"
    ...
    public List<Ruta> leerRuta() {   //este es para obtener varias personas
        ...
        try {
            String consulta = "SELECT IDRUTA, HORARI FROM RUTA";  //poner from siempre para saber que tabla vamos a usar
            PreparedStatement sentencia = Conexion.getConexion().prepareStatement(consulta);
    ...
    public Boolean actualizarRuta(Ruta ruta) { //para actualizar
        try {
            String consulta = "UPDATE RUTA SET IDRUTA= ?, HORARIO = ?"
